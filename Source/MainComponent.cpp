...
public:
    MainContentComponent() : carrierFrequency(440.0), index(0.0), gain (0.0), samplingRate(0.0) {
        ...
        colorOne.setColour(0x1000100, juce::Colours::blue);
                colorOne.addListener(this);
        addAndMakeVisible (colorTwo);
        colorTwo.setColour(0x1000100, juce::Colours::purple);
                colorTwo.addListener(this);
        addAndMakeVisible (colorThree);
        colorThree.setColour(0x1000100, juce::Colours::orange);
    ...
    void buttonClicked (Button* button) override {
        ...
        if(button == &colorOne) {
            modulator.setFrequency(carrierFrequency*(0.9999/2));
        }
        if(button == &colorTwo) {
            modulator.setFrequency(carrierFrequency*(0.9999));
        }
        if(button == &colorThree) {
            modulator.setFrequency(carrierFrequency*(1.9999));
        }
...
private:
    ...
    int samplingRate;
    int nChans;
    bool colorOneSelected;
    bool colorTwoSelected;
    bool colorThreeSelected;
